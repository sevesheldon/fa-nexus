...
$_POST['replace_mode']           = isset($_POST['replace_mode']) ? $_POST['replace_mode'] : "legacy";
...
if ($subsite_id > 1) {
	...
	try {
		DUPX_Log::info("####5");
		DUPX_MU::convertSubsiteToStandalone($_POST['subsite-id'], $dbh, $GLOBALS['DUPX_AC'], $new_content_dir, $_POST['remove_redundant'] );
	} catch (Exception $ex) {
		DUPX_Log::info("####6");
...
if (strlen($_POST['wp_username']) >= 4 && strlen($_POST['wp_password']) >= 6) {
	...
	if ($newuser_count == 0) {
		...
		if ($GLOBALS['DUPX_AC']->mu_mode > 0 && $subsite_id == -1){
			...
			$sql = "INSERT INTO {$GLOBALS['DUPX_AC']->wp_tableprefix}usermeta (user_id, meta_key, meta_value) VALUES ".implode(', ', $sql_values_array);
...
if ($licence_type >= DUPX_LicenseType::Freelancer) {
	if ($_POST['auth_keys_and_salts']) {
		...
		foreach ($need_to_change_const_keys as $const_key) {
			...
			array_push($patterns, "/('|\")".$const_key.".*?\)\s*;/");
			array_push($replace, "'".$const_key."', '{$key}');");
		}
	}
}
