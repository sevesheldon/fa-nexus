...
class SnapLibIOU
{
    ...
    // The value chosen below, makes sure we have a buffer of ~47 million.
    const FileSizeLimit32BitPHP = 2100000000;
    ...
    public static function fseek($handle, $offset, $whence = SEEK_SET)
    {
        ...
        if ($ret_val !== 0) {
            ...
            else if(abs($offset) > self::FileSizeLimit32BitPHP || $offset < 0){
                throw new SnapLib_32BitSizeLimitException("Trying to seek on a file beyond the capability of 32 bit PHP");
            }else {
