...
<div class="toggle-box">
	<a href="javascript:void(0)" onclick="DupPro.Pack.OpenAll()">[<?php DUP_PRO_U::_e('open all');?>]</a> &nbsp;
	<a href="javascript:void(0)" onclick="DupPro.Pack.CloseAll()">[<?php DUP_PRO_U::_e('close all');?>]</a>
</div>
				...
				<div id="dpro-downloads-area">
					<?php if ($error_display == 'none') :?>
						<?php if ($package->contains_storage_type(DUP_PRO_Storage_Types::Local)) :?>
							...
							<button class="button thickbox" onclick="DupPro.Pack.ShowLinksDialog(<?php echo "'{$link_sql}','{$link_archive}','{$link_installer}','{$link_log}'" ;?>);"><i class="fa fa-lock"></i> &nbsp; <?php _e("Share File Links", 'duplicator')?></button>
						<?php else: ?>
							<!-- CLOUD ONLY FILES -->
...
<div class="dup-box-title">
	...
	<?php if($package->Installer->OptsSecureOn) :?>
		<span id="dpro-install-secure-lock" title="<?php DUP_PRO_U::_e('Installer password protection is on for this package.') ?>"><i class="fa fa-lock"></i> </span>
	<?php else : ?>
		<span id="dpro-install-secure-unlock" title="<?php DUP_PRO_U::_e('Installer password protection is off for this package.') ?>"><i class="fa fa-unlock-alt"></i> </span>
	<?php endif; ?>
	<div class="dup-box-arrow"></div>
</div>			
<div class="dup-box-panel" id="dup-package-dtl-install-panel" style="<?php echo $ui_css_install ?>">
	...
	<table class='dpro-dtl-data-tbl'>
		<tr>
			<td colspan="2"><div class="dpro-install-hdr-2"><?php DUP_PRO_U::_e("Setup") ?></div></td>
		</tr>
		<?php if($display_brand===true && $is_freelancer_plus): ?>
			<tr>
				<td><?php DUP_PRO_U::_e("Brand");?>:</td>
				<td><span style="color:#AF5E52; font-weight: bold"><?php echo $brand ?></span></td>
			</tr>
		<?php endif; ?>
		<tr>
			<td><?php DUP_PRO_U::_e("Security");?>:</td>
			...
			<tr>
				<td colspan="2">
					<div id="dpro-pass-toggle">
						...
						<button type="button" id="secure-btn" class="pass-toggle" onclick="DupPro.togglePassword()" title="<?php DUP_PRO_U::_e('Show/Hide Password') ?>"><i class="fa fa-eye"></i></button>
					</div>
				</td>
			</tr>
		<?php endif; ?>
	...
	<table style="width:100%">
		<tr>
			<td colspan="2"><div class="dpro-install-hdr-2"><?php DUP_PRO_U::_e("Prefills") ?></div></td>
		</tr>
	</table>
