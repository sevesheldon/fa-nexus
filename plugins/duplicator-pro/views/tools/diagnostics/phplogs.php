...
if($log_path !== false){
    ...
    if(!is_readable($log_path)){
        $error = sprintf(DUP_PRO_U::__("PHP error log is available on location %s but is not readable. Try setting the permissions to 755."),'<b>'.$log_path.'</b>');
    } else if($error_log === NULL){
        if($log_path > (PHP_INT_MAX / 2))
            $error = sprintf(DUP_PRO_U::__("PHP error log is available on location %s but can't be read because file size is over %s. You must open this file manualy."),'<b>'.$log_path.'</b>', '<b>'.DUP_PRO_U::byteSize($log_path_size).'</b>');
        else
            $error = sprintf(DUP_PRO_U::__("PHP error log is available on location %s but can't be read because some unexpected error. Try to open file manualy and ivestigate all problems what can cause this error."),'<b>'.$log_path.'</b>');
    } else {
    ...
    $error.= sprintf(DUP_PRO_U::__("But if you in any case experience some errors and not see log here, that mean your error log file is placed on some unusual locaton or can't be created because some %s setup. In that case you must open %s file and define %s or call your system administrator to setup it properly."),"<b><i>php.ini</i></b>","<b><i>php.ini</i></b>","<code>error_log</code>").'<br><br>';
...
if($error) : ?>
    <h2><?php
        if($log_path !== false){
            DUP_PRO_U::_e("Log file is found but have error or is unreadable");
        } else {
            DUP_PRO_U::_e("PHP error log not found");
        }
    ?></h2>
    <?php echo $error; ?>
<?php else : ?>
...
<table id="dpro-log-pnls">
		<tr>
            <td id="dpro-log-pnl-left">
                <div class="name"><i class="fa fa-list-alt"></i> <?php echo $log_path; ?></div>
                ...
                <div id="tableContainer" class="tableContainer">
                    <table class="wp-list-table fixed striped" id="error-log">
                        <thead>
                            <tr>
                                <th scope="col" style="width: 10%; text-align: center"><?php DUP_PRO_U::_e('Date'); ?></th>
                                <th scope="col" style="width: 8%; text-align: center"><?php DUP_PRO_U::_e('Type'); ?></th>
                                <th scope="col" style="width: 34%;"><?php DUP_PRO_U::_e('Error'); ?></th>
                                <th scope="col" style="width: 30%;"><?php DUP_PRO_U::_e('File'); ?></th>
                                <th scope="col" style="width: 6%;"><?php DUP_PRO_U::_e('Line'); ?></th>
                            </tr>
                        </thead>
                        <tbody id="the-list"<?php echo (count($error_log) === 0) ? ' style="overflow: hidden;"' : ''; ?>>
                            <?php if(count($error_log) === 0) : ?>
                                <tr style="width:100%; display:table">
                                    <td colspan="5" style="width:100%; display:table"><h3><?php DUP_PRO_U::_e('PHP Error Log is empty.'); ?></h3></td>
                                </tr>
                            <?php else : ?>
                                <?php foreach ($error_log as $line=>$log) : ?>
                                <tr>
                                    <td scope="col" style="width: 15%; text-align: center"><b class="info" title="<?php echo date("Y-m-d H:i:s T (P)", strtotime($log['dateTime'])); ?>"><?php echo $log['dateTime']; ?></b></td>
                                    <td scope="col" style="width: 8%; text-align: center"><?php echo $log['type']; ?></td>
                                    <td scope="col" style="width: 35.5%;">
                                        <?php echo $log['message']; ?>
                                        <?php if(count($log['stackTrace']) > 0) : ?>
                                        <ul>
                                            <li class="title"><?php DUP_PRO_U::_e('Stack trace:'); ?></li>
                                            <?php foreach($log['stackTrace'] as $i=>$trace) : ?>
                                            <li><b>#<?php echo $i; ?></b> <?php echo $trace; ?></li>
                                            <?php endforeach; ?>
                                        </ul>
                                        <?php endif; ?>
                                    </td>
                                    <td scope="col" style="width: 35.5%;"><?php echo $log['file']; ?></td>
                                    <td scope="col" style="width: 6%; text-align: center"><?php echo $log['line']; ?></td>
                                </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
            ...
            <td id="dpro-log-pnl-right">
                ...
                <form id="dup-form-logs" method="post" action="">
                    ...
                    <div class="dpro-opts-items">
                        <strong><?php DUP_PRO_U::_e('PHP Error Filter:'); ?></strong>
                        <select type="text" id="filter" name="filter" style="width:100%;">
                            <option value="">--- <?php DUP_PRO_U::_e('None'); ?> ---</option>
                            <?php
                            	foreach (array(
							...
							?>
                            <option value="<?php echo $option; ?>"<?php echo ($filter == $option ? ' selected':''); ?>><?php echo $name; ?></option>
                            <?php endforeach; ?>
                        </select>
                        <hr>
                ...
                <div style="color:#cc0000">
                    <i class="fa fa-info-circle"></i> <?php printf(DUP_PRO_U::__("You see only last %s logs inside %s file."), $line, DUP_PRO_PHP_Log::get_filename($log_path)); ?>
                </div>
                <?php endif; ?>
                <?php if(isset($line)) : ?>
                <br>
                <form id="dup-form-clear-log" method="post" action="">
                    <button class="button" type="button" onclick="return DupPro.Tools.ClearLog();"><?php DUP_PRO_U::_e('Clear Log'); ?></button>
