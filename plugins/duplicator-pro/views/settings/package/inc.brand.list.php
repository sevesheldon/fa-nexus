...
<div <?php echo ($is_freelancer_plus) ? "style='display:none'" : ""; ?>>
	<h2><?php DUP_PRO_U::_e("Installer Branding") ?></h2>
	<hr size="1"/>
	
	<div style="width:850px">
		<?php
			DUP_PRO_U::_e("Create your own WordPress distribution by adding a custom name and logo to the installer!  "
		...
		<br/><br/>
		<b>
			<?php DUP_PRO_U::_e("This feature is only available in Freelancer, Business or Gold licenses. For details on how to upgrade your license "); ?>
		</b>
		<a href="https://snapcreek.com/duplicator/docs/faqs/#faq-presale-035-q" target="_blank"><?php DUP_PRO_U::_e("click here"); ?></a>. <br/>
	</div>
    ...
    <tr>
        <td>
            <select id="bulk_action">
                ...
                <option value="delete" title="<?php DUP_PRO_U::_e('Delete selected brand endpoint(s)'); ?>"><?php _e("Delete"); ?></option>
            </select>
...
<table class="widefat brand-tbl">
	<thead>
		<tr>
			...
			<th style='width:300px;'><?php DUP_PRO_U::_e('Name'); ?></th>
	...
	<tbody>
		...
		foreach ($brands as $x=>$brand) :
			...
			<tr id='main-view-<?php echo $brand->id ?>' class="brand-row<?php echo ($i % 2) ? ' alternate' : ''; ?>">
				...
				<td>
					...
					<?php if ($brand->editable) : ?>
						<div class="sub-menu">
							<a href="javascript:void(0);" onclick="DupPro.Settings.Brand.Edit('<?php echo $brand->id; ?>')"><?php DUP_PRO_U::_e('Edit') ?></a> |
							<a href="javascript:void(0);" onclick="DupPro.Settings.Brand.View('<?php echo $brand->id; ?>');"><?php DUP_PRO_U::_e('Quick View') ?></a>
                            <?php if(!$brand->active) : ?>|
                            <a href="<?php echo get_admin_url(null, "admin.php?page=duplicator-pro-settings&tab=package&sub=brand&made_active=true&id=".$brand->id ); ?>" style="color:orangered;">
                                <?php DUP_PRO_U::_e('Set active'); ?>
                            </a>
                            <?php endif; ?> |
							<a href="javascript:void(0);" onclick="DupPro.Settings.Brand.Delete('<?php echo $brand->id; ?>');"><?php DUP_PRO_U::_e('Delete') ?></a>
						</div>
					<?php else : ?>
						<div class="sub-menu">
							<a href="javascript:void(0);" onclick="DupPro.Settings.Brand.Edit(0)"><?php DUP_PRO_U::_e('View') ?></a> |
							<a href="javascript:void(0);" onclick="DupPro.Settings.Brand.View('<?php echo $brand->id; ?>');"><?php DUP_PRO_U::_e('Quick View') ?></a>
						</div>
					<?php endif; ?>
				</td>
				<td>
					<?php if ($brand->active) : ?>
						<span class="fa fa-bolt" style="color:green;"></span> <?php echo DUP_PRO_U::_e('Enabled'); ?>
					<?php endif; ?>
			...
			<tr id='quick-view-<?php echo $brand->id ?>' class='<?php echo ($i % 2) ? 'alternate ' : ''; ?>brand-detail'>
				<td colspan="3">
					...
					<div>
						<label><?php DUP_PRO_U::_e('Logo') ?>:</label>
						<?php echo $brand->logo ?>
					</div>
        ...
        <tr id='main-view-<?php echo $brands[0]->id; ?>' class="brand-row">
            ...
            <td>
                ...
                <div class="sub-menu">
                    <a href="javascript:void(0);" onclick="DupPro.Settings.Brand.Edit(0)"><?php DUP_PRO_U::_e('View'); ?></a> |
                    <a href="javascript:void(0);" onclick="DupPro.Settings.Brand.View('<?php echo $brands[0]->id; ?>');"><?php DUP_PRO_U::_e('Quick View'); ?></a>
                    <?php if(!$is_default_active) : ?>|
                    <a href="<?php echo get_admin_url(null, "admin.php?page=duplicator-pro-settings&tab=package&sub=brand&made_active=true&id=" . $brands[0]->id ); ?>" style="color:orangered;"><?php DUP_PRO_U::_e('Set active'); ?></a>
                    <?php endif; ?>
                </div>
            </td>
            <td>
        ...
        <tr id="quick-view-<?php echo $brands[0]->id ?>" class="brand-detail">
            <td colspan="3">
                ...
                <button type="button" class="button" onclick="DupPro.Settings.Brand.View('<?php echo $brands[0]->id; ?>');"><?php DUP_PRO_U::_e('Close') ?></button>
            </td>
        </tr>
