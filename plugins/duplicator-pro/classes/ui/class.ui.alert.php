    ...
    public static function failedScheduleCheck()
    {
        ...
        if(($system_global !== null) && ($system_global->schedule_failed)) {
           ...
           // $clear_url = self_admin_url()."admin.php?page=".DUP_PRO_Constants::$SCHEDULES_SUBMENU_SLUG.'&dup_pro_clear_schedule_failure=1';
            ...
            sprintf(DUP_PRO_U::__('%sWarning! A Duplicator Pro scheduled backup has failed.%s'),'<b>','</b> <br/>') .
            sprintf(DUP_PRO_U::__('This message will continue to be displayed until a %sscheduled build%s successfully runs'), "<a href='admin.php?page=duplicator-pro-schedules'>", '</a>') .
			sprintf(DUP_PRO_U::__('To ignore and clear this message %sclick here%s'), "<a href='{$clear_url}'>", '</a>.<br/></p></div>');
        }    
    }
